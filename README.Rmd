...
Note that the `hillshader` package includes the `maungawhau` and `maungawhau_hr` datasets. These are geo-referenced, raster datasets. `maungawhau` corresponds to the well-known `volcano` dataset. It is a $87 \times 61$ elevation  matrix for Maungawhau, one of the circa 80 volcanoes in the Auckland volcano filed, in Aotearoa/New Zealand. The `maungawhau_hr` dataset is a "high-resolution" version of that dataset, and is a 1 m resolution, $860 \times 600$ elevation matrix derived from a LiDAR dataset collected by LINZ. 
...
The `hillshader` function accept a `shader` option, with is a list of the sucessive shader functions to apply to create the hillshade layer. The accepeted values must be `rayshader` shader functions (`ray_shade`, `ambient_shade`, `lamb_shade`), and the order is important.
